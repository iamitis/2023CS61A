...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    """
    "*** YOUR CODE HERE ***"
    return tree(lokis_replacement if is_leaf(t) and label(t) == 'loki' else label(t), [replace_loki_at_leaf(b, lokis_replacement) for b in branches(t)])
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    return pow(pow(get_lat(city_a) - get_lat(city_b), 2) + pow(get_lon(city_a) - get_lon(city_b), 2), 1/2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_a) if distance(here, city_a) <= distance(here, city_b) else get_name(city_b)
...
def hailstone_tree(n, h):
    ...
    if n > 4 and (n - 1) % 3 == 0 and (n - 1) / 3 % 2 != 0:
