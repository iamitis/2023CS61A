...
def interleaved_sum(n, odd_term, even_term):
    ...
    def sum(k, n, f, g):
        if k > n:
            return 0
        return f(k) + sum(k + 1, n, g, f)
    return sum(1, n, odd_term, even_term)
...
def count_coins(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count(total, maxValue):
        if maxValue == 1:
            return 1
        if total < 0:
            return 0
        return count(total - maxValue, maxValue) + count(total, next_smaller_coin(maxValue))
